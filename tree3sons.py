...
class tree3sons():
    def __init__(self):
        ...
        self.min_delta_time = 1

    def __str__(self):
        return f"Max engineers at the same time: {self.max_engineers}  \nTime frame/s: {self.list_periods_of_time}"

    def print_solution(self):
        print(self)

    def push_list_of_tasks(self, list_of_tasks, is_dates_str_format = False,datetime_str_format = '%d/%m/%y %H:%M',min_delta_time=60):   #min_delta_time=60 = one minute
        ...
        while not all_task_pushed:
            if start < curr_node.start:
                if stop < curr_node.start:
                    ...
                    self.__go_left(start, curr_node.start - self.min_delta_time, engineers,curr_node)
                ...
                if stop <= curr_node.stop:
                    ...
                    start = curr_node.stop + self.min_delta_time
